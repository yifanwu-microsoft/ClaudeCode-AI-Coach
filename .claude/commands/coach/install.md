...
1. 询问用户："请选择安装语言 / Please select installation language"
2. 提供两个选项：
   - 中文
   - English
3. 根据用户选择，记住对应的 `--lang` 参数：
   - 中文 → `--lang zh`
   - English → `--lang en`
...
- 检查 `~/.claude/commands/coach/assess.md` 是否存在
...
**首次安装**（安装脚本输出中包含"已创建"）：
- 告知用户：✅ 安装成功！教练系统已全局生效
- 建议：现在可以去任何项目中打开 Claude Code，输入 `/coach:assess` 进行首次评估
...
- 如需重置进度，手动删除 `~/.claude/PROGRESS.md` 后重新执行 `/coach:install`
